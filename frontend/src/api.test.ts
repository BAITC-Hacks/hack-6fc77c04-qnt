...
describe('contract fixtures', () => {
 it.each(examples)('$title preserves response invariants', ({ request }) => {
  ...
  expect(result.rejections.reduce((sum, item) => sum + item.count, result.eligible_count)).toBe(result.total_in_category);
  expect(result.rejections.map(r => r.code)).toEqual(['busy', 'format', 'budget', 'language', 'duration']);
  expect(result.cards.every(c => c.explanation_mode === 'local' && c.flags.synthetic)).toBe(true);
 });
 it('changes the date fixture and remains deterministic', () => {
  ...
  expect(demoMatch({ ...request, date: '2026-10-11' }).cards).not.toEqual(demoMatch(request).cards);
 });
...
describe('real API transport', () => {
 ...
 it.each([422, 503])('shows server error %s without demo fallback', async status => {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ error: { message: 'Тестовая ошибка' } }), { status })));
  await expect(api('match', new AbortController().signal, examples[0].request)).rejects.toThrow('Тестовая ошибка');
 });
 it('reports non-JSON replies', async () => {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('<html>error</html>')));
  await expect(api('options', new AbortController().signal)).rejects.toThrow('неизвестном формате');
 });
 it('reports network failure', async () => {
  vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new TypeError('Failed to fetch')));
  await expect(api('options', new AbortController().signal)).rejects.toThrow('связаться с API');
 });
 it('reports timeout', async () => {
  vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new DOMException('timeout', 'TimeoutError')));
  await expect(api('options', new AbortController().signal)).rejects.toThrow('12 секунд');
 });
});
