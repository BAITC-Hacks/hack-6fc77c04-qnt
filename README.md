...
Первые версии сервера Естая (`b3c498b`) и интерфейса Бекзат (`cfa205a`) объединены с сохранением обоих авторов; интегрированы README-материалы и скриншоты Бекзат (`a343e7f`). Локально прошли **94 серверных теста**, **29 тестов frontend**, production-сборка и браузерная проверка с настоящим API. Офлайн-проверка охватила **30 600 комбинаций** по всем 100 дням каталога. Публичные релизы и границы проверки зафиксированы в [отчёте интеграции](docs/INTEGRATION_QA.md); это не обещание постоянной доступности стенда.
...
Это условные ориентиры выбора, а не приписанные заказчику пожелания или гарантии результата. AI выбирает доказательство из описания, код превращает его в краткое объяснение. Бюджет, язык, длительность и календарь видны отдельно в фактах. При слабом описании система честно показывает ограниченность оснований. Полезность индивидуального объяснения важнее количества упоминаний AI.

## Как это выглядит

Новый дизайн Бекзат: фотографический фон, светлая форма и горизонтальные карточки с отдельными объяснениями и цитатами. Сохраняются AI/local, русские подписи фактов и видимая причина отсева по занятости. Фоновое изображение создано AI для оформления, не представляет реального подрядчика.
...
3. В разделе **«Смета»** видны выбранные услуги и сумма начальных цен. **«Заменить»** возвращает исходные условия этой позиции в форму; старая позиция остаётся до выбора новой.
4. **«Скачать смету»** сохраняет TXT с позициями, условиями, отметками данных и итогом. Черновик также хранится локально в браузере; при запрете хранилища интерфейс сообщает об этом.
...
Смета относится только к выбранным услугам, не является полной стоимостью мероприятия, заявкой или бронированием. Сохранённые цены и занятость автоматически не обновляются; перед решением нужно повторить подбор и подтвердить условия у подрядчиков. Функция не меняет исходные данные, API, фильтры и порядок выдачи, не вызывает AI дополнительно. UI-демо хранит смету только в памяти и не перезаписывает реальный черновик.
...
[Полный протокол](docs/ai-eval-results.json) содержит параметры, реальные snapshots, токены, задержки, цитаты, ответы и хеши исходников. У обоих моделей повтор каждого запроса совпал и обошёлся без сетевого вызова. У одного профиля недостаточно конкретных фактов для максимальной оценки. Уникальность цитаты — необходимый, но недостаточный признак полезности объяснения. Выборка ручная, одна попытка на запрос/модель: это не p95, нагрузочный тест или слепое сравнение людьми.
