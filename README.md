...
Первые версии сервера Естая (`b3c498b`) и интерфейса Бекзат (`cfa205a`) объединены с сохранением обоих авторов; интегрированы README-материалы и скриншоты Бекзат (`a343e7f`). Локально прошли **82 серверных теста**, **17 тестов frontend**, production-сборка и браузерная проверка с настоящим API. Офлайн-проверка охватила **30 600 комбинаций** по всем 100 дням каталога. Публичные релизы и границы проверки зафиксированы в [отчёте интеграции](docs/INTEGRATION_QA.md); это не обещание постоянной доступности стенда.
...
Текущая подача: отдельные счётчики, «Почему подходит», заметный режим AI/local, цитата и русские подписи фактов. Новый снимок — настоящий локальный API без AI, ширина 1280 px. Публичное демо может отставать от текущего Git. [Проверка изменений и ограничений](docs/EXPLANATION_QA.md).

![Короткое объяснение и читаемые факты](docs/assets/evidence-results.png)
