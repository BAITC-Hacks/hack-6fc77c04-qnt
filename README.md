...
Текущая подача: отдельные счётчики, «Почему подходит», заметный режим AI/local, цитата и русские подписи фактов. Снимок ниже — настоящий локальный API без AI, ширина 1280 px; объяснения дополнены ориентирами выбора. Публичное демо может отставать от текущего Git. [Проверка изменений и ограничений](docs/EXPLANATION_QA.md).

![Короткое объяснение и читаемые факты](docs/assets/decision-results.png)
...
Бекзат владеет `frontend/**` и `docs/UI_QA.md`; Естай — сервером, каталогом и деплоем. Общий контракт не меняется односторонне. Клиент использует относительный `/api` и dev-прокси на порт 8000. Промпт Бекзат не изменён. Её новый дизайн в `feat/contractor-ui` (`4bed3d1`) пока не интегрирован по просьбе команды.
